...
module Surveyor
  ...
  class RedcapParser
    ...
    def missing_columns(r)
      missing = []
      missing << 'choices_or_calculations' unless r.map(&:to_s).include?('choices_or_calculations') || r.map(&:to_s).include?('choices_calculations_or_slider_labels')
      missing << 'text_validation_type' unless r.map(&:to_s).include?('text_validation_type') || r.map(&:to_s).include?('text_validation_type_or_show_slider_number')
    ...
    def resolve_references
      context[:dependency_conditions].each do |dc|
        ...
        if dc.answer_reference.blank? && (context[:question_references][dc.question_reference].answers.size == 1)
...
module SurveyorRedcapParserAnswerMethods
  def build_and_set(context, r)
    ...
    (r[:choices_or_calculations] || r[:choices_calculations_or_slider_labels]).to_s.split('|').each do |pair|
      ...
      if aref.blank? || atext.blank? || (aref.to_i.to_s != aref)
        ...
        unless context[:question].reference_identifier.blank? || aref.blank? || !context[:answer].valid?
