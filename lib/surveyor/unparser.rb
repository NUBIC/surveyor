# note that `frozen_string_literal: true` breaks this as it is right now
...
module Surveyor
  class Unparser
    ...
    def self.hash_diff(h1, h2)
      h1.dup.delete_if { |k, v| h2[k] == v }.merge!(h2.dup.delete_if { |k, _v| h1.has_key?(k) })
    end
  end
end
...
class Survey < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at inactive_at id title access_code api_id).include? k }.symbolize_keys!
    dsl << "survey \"#{title}\""
...
class SurveySection < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at id survey_id).include? k }.symbolize_keys!
    group_questions = []
    dsl << "  section \"#{title}\""
    dsl << (attrs.blank? ? " do\n" : ", #{attrs.inspect.gsub(/\{|\}/, '')} do\n")
    questions.each_with_index do |question, index|
      if question.solo?
        ...
        if (index + 1 >= questions.size) || (question.question_group != questions[index + 1].question_group)
...
class QuestionGroup < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at id api_id).include?(k) or (k == 'display_type' && %w(grid repeater default).include?(v)) }.symbolize_keys!
...
class Question < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at reference_identifier id survey_section_id question_group_id api_id).include?(k) or (k == 'display_type' && v == 'label') }.symbolize_keys!
    dsl << (solo? ? "\n" : '  ')
    dsl << if display_type == 'label'
      '    label'
    else
      '    q'
    end
    ...
    dsl << " \"#{text}\""
    dsl << (attrs.blank? ? "\n" : ", #{attrs.inspect.gsub(/\{|\}/, '')}\n")
    if solo? || (question_group.display_type != 'grid')
      answers.each { |answer| answer.unparse(dsl) }
    end
    dependency&.unparse(dsl)
  end
end
class Dependency < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at id question_id).include?(k) }.symbolize_keys!
...
class DependencyCondition < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at question_id question_group_id rule_key rule operator id dependency_id answer_id).include? k }.symbolize_keys!
...
class Answer < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at reference_identifier response_class id question_id api_id).include? k }.symbolize_keys!
    ...
    if response_class.to_s.titlecase == text && attrs == { display_type: 'hidden_label' }
      ...
      dsl << [text.blank? ? nil : text == 'Other' ? ' :other' : text == 'Omit' ? ' :omit' : " \"#{text}\"",
              response_class.blank? || (response_class == 'answer') ? nil : " #{response_class.to_sym.inspect}",
...
class Validation < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at id answer_id).include?(k) }.symbolize_keys!
...
class ValidationCondition < ActiveRecord::Base
  ...
  def unparse(dsl)
    ...
    attrs = attributes.delete_if { |k, v| with_defaults[k] == v or %w(created_at updated_at operator rule_key id validation_id).include? k }.symbolize_keys!
