...
module Surveyor
  ...
  class Parser
    ...
    def self.parse_file(filename, options = {})
      parse(File.read(filename), { filename: filename }.merge(options))
    end
    ...
    def self.raise_error(str, skip_trace = false)
      ...
      raise Surveyor::ParserError, "#{source_extract(line)}\nline \##{line}: #{str}\n"
    end
    ...
    def report_missing_default_locale
      if !context[:survey].translations.empty? &&
          context[:survey].translations.select { |t| YAML::safe_load(t.translation, [Symbol]) == {} }.empty?
        Surveyor::Parser.raise_error('No default locale specified for translations.', true)
      end
    end

    def report_lost_and_duplicate_references
      Surveyor::Parser.raise_error("Bad references: #{context[:bad_references].join('; ')}", true) unless context[:bad_references].empty?
      Surveyor::Parser.raise_error("Duplicate references: #{context[:duplicate_references].join('; ')}", true) unless context[:duplicate_references].empty?
    end

    def resolve_question_correct_answers
      context[:questions_with_correct_answers].each do |question_reference_idenitifer, correct_answer_reference|
        # Looking up references for quiz answers
        if context[:answer_references][question_reference_idenitifer] &&
             (a = context[:answer_references][question_reference_idenitifer][correct_answer_reference]) &&
             a.save
          context[:question_references][question_reference_idenitifer].correct_answer_id = a.id
        else
          context[:bad_references].push "q_#{question_reference_idenitifer}.correct => a_#{correct_answer_reference}"
        end
      end
    end
    ...
    def resolve_dependency_condition_references
      context[:dependency_conditions].each do |dc|
        ...
        context[:bad_references].push "q_#{dc.question_reference}" unless (dc.question = context[:question_references][dc.question_reference])
        context[:answer_references][dc.question_reference] ||= {}
        context[:bad_references].push "q_#{dc.question_reference}, a_#{dc.answer_reference}" if !dc.answer_reference.blank? && (dc.answer = context[:answer_references][dc.question_reference][dc.answer_reference]).nil?
      end
    end

    def resolve_skip_logic_references
      context[:skip_logics].each do |sl|
        # Looking up references to sections for linking the skip logics target
        context[:bad_references].push "s_#{sl.target_survey_section_reference}" if !sl.target_survey_section_reference.blank? && (sl.target_survey_section = context[:survey].sections.detect { |s| s.reference_identifier == sl.target_survey_section_reference }).nil?
      end
      context[:skip_logic_conditions].each do |slc|
        # Looking up references to questions and answers for linking the condition objects
        context[:bad_references].push "q_#{slc.question_reference}" unless (slc.question = context[:question_references][slc.question_reference])
        context[:answer_references][slc.question_reference] ||= {}
        context[:bad_references].push "q_#{slc.question_reference}, a_#{slc.answer_reference}" if !slc.answer_reference.blank? && (slc.answer = context[:answer_references][slc.question_reference][slc.answer_reference]).nil?
      end
    end
  end
end
...
module SurveyorParserSurveyMethods
  def parse_and_build(context, args, _original_method, reference_identifier)
    # clear context
    clear(context)
...
module SurveyorParserSurveyTranslationMethods
  def parse_and_build(context, args, _original_method, _reference_identifier)
...
module SurveyorParserQuestionMethods
  def parse_and_build(context, args, original_method, reference_identifier)
    ...
    context[:questions_with_correct_answers][self.reference_identifier] = correct unless self.reference_identifier.blank? || correct.blank?
    ...
    if context[:question_group] && context[:question_group].display_type == 'grid'
      (context[:grid_answers] || []).each do |grid_answer|
        a = context[:question].answers.build(PermittedParams.new(grid_answer.attributes.reject { |k, _v| %w(id api_id created_at updated_at).include?(k) }).answer)
        context[:answer_references][self.reference_identifier] ||= {} unless self.reference_identifier.blank?
        context[:answer_references][self.reference_identifier][grid_answer.reference_identifier] = a unless self.reference_identifier.blank? || grid_answer.reference_identifier.blank?
      end
    end
  end
end
...
module SurveyorParserAnswerMethods
  def parse_and_build(context, args, _original_method, reference_identifier)
    ...
    if context[:question_group] && context[:question_group].display_type == 'grid'
      ...
      # keep reference for dependencies
      unless context[:question].reference_identifier.blank? || self.reference_identifier.blank?
