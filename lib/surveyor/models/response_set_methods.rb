  ...
  module Models
    module ResponseSetMethods
      ...
      def to_csv(print_header=true)
        ...
        FCSV(result = "") do |csv|
          if print_header
            csv << ["response set access code"] + qcols.map{|qcol| "question.#{qcol}"} + acols.map{|acol| "answer.#{acol}"} + rcols.map{|rcol| "response.#{rcol}"}
          end
          responses.each do |response|
            csv << [self.access_code] + qcols.map{|qcol| response.question.send(qcol)} + acols.map{|acol| response.answer.send(acol)} + rcols.map{|rcol| response.send(rcol)}
          end
        end
        result
      end
