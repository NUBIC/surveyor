  ...
  module Models
    module SkipLogicConditionMethods
      ...
      included do
        ...
        belongs_to :skip_logic, inverse_of: :skip_logic_conditions, optional: false
      ...
      def to_hash(response_set)
        ...
        responses =
          if question.blank?
            ...
            response_set.responses.select { |r| r.answer_id.in?(question.answer_ids) }
          end
        ...
        if operator.match /^count(>|>=|<|<=|==|!=)\d+$/
          ...
          klass = 'answer' if as(klass).nil?
          case operator
          when '==', '<', '>', '<=', '>='
            # logger.warn( {rule_key.to_sym => response.as(klass).send(self.operator, self.as(klass))})
            return {
              rule_key.to_sym =>
                !response.as(klass).nil? && response.as(klass).send(operator, as(klass)),
            }
          when '!='
