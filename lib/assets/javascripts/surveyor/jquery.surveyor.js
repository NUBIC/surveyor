...
jQuery(document).ready(function(){
  ...
  function successfulSave(responseText) {
    ...
    // and insert/remove ids
    // e.g. {"ids": {"2" => 234}, "remove": {"4" => 21}, "hide":["question_12","question_13"],"show":["question_14"]}
    jQuery.each(responseText.show, function(){ showElement(this) });
    jQuery.each(responseText.hide, function(){ hideElement(this) });
    jQuery.each(responseText.ids, function(k,v){
      jQuery('#r_'+k+'_question_id').
        after('<input id="r_'+k+'_id" type="hidden" value="'+v+'" name="r['+k+'][id]"/>');
    });
    jQuery.each(responseText.remove, function(k,v){
      jQuery('#r_'+k+'_id[value="'+v+'"]').remove();
    });
    return false;
  }
