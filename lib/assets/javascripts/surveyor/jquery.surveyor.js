...
jQuery(document).ready(function(){
  ...
  jQuery('.check_boxes_plus input[type=text]').change(function(){
  ...
  jQuery('.radio_buttons_plus input[type=text]').change(function(){
    var textValue = $(this).val()
    if (textValue.length > 0) {
  ...
  jQuery("form#survey_form input, form#survey_form select, form#survey_form textarea").change(function(){
    ...
    if ($(that).is('input[data-input-mask]')) {
      ...
      $(that).data('surveyor-change-timer', setTimeout(update));
    } else {
      update();
    }
  });
  ...
  $('div.q_slider select').each(function(i,e) {
  ...
  $('input.exclusive:checked').parents('div[id^="q_"]').
    find(':checkbox').
    not(".exclusive").
    attr('checked', false).
    attr('disabled', true);
  ...
  $('input.exclusive:checkbox').click(function(){
    var e = $(this);
    var others = e.parents('div[id^="q_"]').find(':checkbox').not(e);
    if(e.is(':checked')){
