...
jQuery(document).ready(function(){
  ...
  jQuery('fieldset.q_slider select').each(function(i,element) {
		var intialValue = $(element)[ 0 ].selectedIndex
		var max = element.length - 1
    var slider = $( "<div id='slider'></div>" ).insertAfter( element )
		var tooltip = $("<span class='tooltip-slider'></span>").insertBefore( slider )
		
	 	tooltip.text($(element).children(':selected').text());
		tooltip.css('left', (slider.width() / max)  * intialValue - tooltip.width()/2);
      ...
      range: "min",
      value: intialValue,
			start: function(event,ui) {		tooltip.removeClass('invisible'); },			    
      slide: function( event, ui ) {
			 var offset  =  (slider.width() / max) * ui.value
       $(element)[ 0 ].selectedIndex = ui.value;
			 tooltip.text($(element).children(':selected').text());
			 tooltip.css('left', offset - tooltip.width()/2);
      },
			stop: function() {
				$(element).trigger('change');
				tooltip.addClass('invisible');
			},
			tickInterval: Math.floor((element.length - 1) / 3),
			tickLabels: $.map(element, function(option) {return option.text;}),
    });
  }).hide();
