...
jQuery(document).ready(function(){
  ...
  jQuery(document).on("click", "#survey_form input[type='submit']", function(e){
    var last_question = $("input[type='text']")[0];
    if (last_question) {
      e.preventDefault();
      question_data = $(last_question).parents('fieldset[id^="q_"],tr[id^="q_"]').
        find("input, select, textarea").
        add($("form#survey_form input[name='authenticity_token']")).
        serialize();
      $.ajax({
        type: "PUT",
        url: $(last_question).parents('form#survey_form').attr("action"),
        data: question_data, dataType: 'json',
        success: function(response) {
          successfulSave(response);
          //trigger validations after evaluating last question
          $("#survey_form input[type='submit']").submit();
        }
      });
    }
  });
  ...
  function removeUselesResponse(id){
    $.ajax({
      type: "DELETE",
