...
jQuery(document).ready(function(){
  ...
  function highlightErrors(responseText) {
    $.each(responseText.errors, function(k,v){
      $('#q_' + v.question + '_error').remove();
      $('#q_' + v.question).addClass('error');
      $('#q_' + v.question).append('<div id="q_' + v.question + '_error">' + v.message + '</div>');      
    });
  }
  ...
  function successfulSave(responseText){ // for(key in responseText) { console.log("key is "+[key]+", value is "+responseText[key]); }
    ...
    jQuery.each(responseText.correct, function(){ 
      jQuery('#q_' + this).removeClass('error'); 
      jQuery('#q_' + this + '_error').remove(); 
    });
